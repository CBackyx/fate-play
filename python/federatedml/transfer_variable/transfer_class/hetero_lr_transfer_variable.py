...
class HeteroLRTransferVariable(BaseTransferVariables):
    def __init__(self, flowid=0):
        ...
        self.paillier_pubkey = self._create_variable(name='paillier_pubkey', src=['guest', 'host'], dst=['guest', 'host'])
        self.share_weights = self._create_variable(name='share_weights', src=['guest', 'host'], dst=['guest', 'host'])
        self.share_protocol = self._create_variable(name='share_protocol', src=['guest', 'host'], dst=['guest', 'host'])
        # self.guest_paillier_pubkey = self._create_variable(name='guest_paillier_pubkey', src=['guest'], dst=['host'])
